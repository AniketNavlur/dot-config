...
local KEYMAPS = {
    ...
    ["git-keymaps"] = {
        ...
        cmd.bind("v", "<leader>gu", "git.unstage_selected"):add_desc("git unstage range"),
        cmd.bind("v", "<leader>gr", "git.revert_selected"):add_desc("git revert range"),
        cmd.bind("n", "<leader>ga", "git.stage_hunk"):add_desc("git stage hunk"),
        cmd.bind("n", "<leader>gu", "git.unstage_hunk"):add_desc("git unstage hunk"),
        cmd.bind("n", "<leader>gr", "git.revert_hunk"):add_desc("git revert hunk"),
        ...
        cmd.bind("n", "<leader>gR", "git.revert_file"):add_desc("git revert file"),
        cmd.bind("n", "<leader>gd", "git.diff"):add_desc("git diff"),
        cmd.bind("n", "<leader>gs", "git.status"):add_desc("git status"),
        cmd.bind("n", "<leader>gc", "git.commit"):add_desc("git commit"),
    ...
    ["dap-keymaps"] = {
        ...
        cmd.bind("n", "<leader>dr", "workbench.debug.action.step_over"):add_desc("step over"),
    ...
    for _, mapping in pairs(keymaps) do
        if mapping.prefix then
            for _, m in pairs(mapping.mode) do
                _ = has_wk and
                    wk.add({mapping.keys, group = mapping.prefix, mode = m, buffer = opts.buffer or nil })
            end
        end
